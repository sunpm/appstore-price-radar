...
const emit = defineEmits<{
  'update:historyTargetPrice': [value: string];
  saveTargetPrice: [];
}>();
...
const onTargetPriceInput = (event: Event) => {
  ...
  emit('update:historyTargetPrice', value);
};
...
const selectedTargetRule = computed(() => {
...
const dayChangeClass = (value: number | null) => {
...
<template>
  ...
  >
    <div class="flex flex-wrap items-center justify-between gap-3">
      <h2 class="text-lg font-semibold tracking-tight text-zinc-900">价格趋势分析（按日最低价聚合）</h2>
      <p v-if="props.selectedHistory" class="text-sm text-zinc-500">{{ props.selectedAppLabel }}</p>
    </div>

    <div class="mt-4 rounded-2xl border border-zinc-200/80 bg-zinc-50/70 p-4">
      <div class="grid gap-3 lg:grid-cols-[1fr_auto] lg:items-end">
        <div>
          <p class="text-sm font-medium text-zinc-700">当前通知规则</p>
          <p class="mt-1 text-sm text-zinc-600">{{ selectedTargetRule }}</p>
        </div>
        <form class="grid w-full gap-2 sm:grid-cols-[1fr_auto] lg:w-auto" @submit.prevent="emit('saveTargetPrice')">
          <input
            :value="props.historyTargetPrice"
            type="number"
            min="0.01"
            step="0.01"
      ...
      <p class="mt-2 text-xs text-zinc-500">可在此直接更新当前应用的通知阈值，规则为“当前价 <= 目标价格”。</p>
    </div>

    <div v-if="props.loadingHistory" class="mt-4 grid gap-3">
      <div class="skeleton-box h-28 rounded-2xl"></div>
      <div class="skeleton-box h-56 rounded-2xl"></div>
    </div>
    ...
    <div v-else class="mt-4">
      ...
      <div class="mt-4 max-h-[360px] overflow-auto rounded-2xl border border-zinc-200">
        <table class="min-w-full border-collapse bg-white text-left text-sm">
          <thead>
            <tr>
              <th class="sticky top-0 border-b border-zinc-200 bg-zinc-50 px-3 py-2 text-xs font-medium uppercase tracking-[0.12em] text-zinc-600">日期</th>
              <th class="sticky top-0 border-b border-zinc-200 bg-zinc-50 px-3 py-2 text-xs font-medium uppercase tracking-[0.12em] text-zinc-600">当日最低价</th>
              <th class="sticky top-0 border-b border-zinc-200 bg-zinc-50 px-3 py-2 text-xs font-medium uppercase tracking-[0.12em] text-zinc-600">日变动</th>
              <th class="sticky top-0 border-b border-zinc-200 bg-zinc-50 px-3 py-2 text-xs font-medium uppercase tracking-[0.12em] text-zinc-600">采样次数</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="row in dailyRows" :key="`${row.day}-${row.price}`" class="border-b border-zinc-100">
              <td class="px-3 py-2">{{ dayLabel(row.day) }}</td>
              <td class="px-3 py-2">{{ props.toMoney(row.price, row.currency) }}</td>
              <td :class="['px-3 py-2 font-medium', dayChangeClass(row.dayChangePct)]">
                {{ toPercent(row.dayChangePct) }}
              </td>
              <td class="metric-mono px-3 py-2">{{ row.sampleCount }}</td>
            </tr>
          </tbody>
        </table>
      </div>
