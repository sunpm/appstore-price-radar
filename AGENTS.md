...
- 同一块内容只保留一层主标题，不要再叠加同义的小标题、 eyebrow、区块标签或重复命名，例如“任务列表 / 我的监控任务”“新建任务 / 新建监控任务”“价格轨迹 / 价格轨迹”这类写法应直接避免。
- 状态文案只在缺少它会影响理解时才出现；不要单独堆“账号状态 / 已登录”“当前状态 / 结果按最近捕捉时间排序”这类弱信息密度说明，能并入现有标题、字段或操作区就不要额外生成一层。
...
- 不要在一个区块顶部同时放“标签 + 主标题 + 说明文字”三层结构；默认只保留主标题，只有确实承担筛选、计数、更新时间、当前价格等信息时，才在同一行补充必要辅助信息。
- 组件内如果已经有明确标题，就不要再重复渲染同义 chip、区块眉题、辅助标题或说明句；信息必须前进，不能只是换个说法重复一遍。
