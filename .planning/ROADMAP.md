...
- [ ] **Phase 1: 数据基线与可部署性** - 统一迁移、部署文档与 fresh install 验证路径
- [ ] **Phase 2: 价格数据正确性** - 修正采集与落库一致性，确保价格和提醒可信
- [ ] **Phase 3: 调度与认证安全** - 为巡检和认证入口补互斥、限流和强制鉴权
- [ ] **Phase 4: 契约与前端稳态** - 收敛 DTO、统一前端鉴权处理并优化历史查看体验
...
Plans:
- [ ] 02-01: 重构 App Store 价格解析与无效数据分支
- [ ] 02-02: 为单应用刷新增加一致性边界与持久化测试
- [ ] 02-03: 统一即时刷新与巡检共享规则
...
Plans:
- [ ] 03-01: 为巡检增加互斥机制和任务级统计
- [ ] 03-02: 为认证链路增加限流、失败次数和凭证淘汰策略
- [ ] 03-03: 强化 `/api/jobs/check` 的生产鉴权与配置保护
- [ ] 03-04: 补调度与认证高风险边界测试

### Phase 4: 契约与前端稳态
**Goal**: 统一前后端契约和前端鉴权请求行为，降低页面抖动与字段漂移。
**Depends on**: Phase 3
**Requirements**: AUTH-04, API-01, API-02, API-04
**Success Criteria** (what must be TRUE):
...
Plans:
- [ ] 04-01: 收敛认证与订阅 DTO 契约
- [ ] 04-02: 提取共享前端鉴权请求与错误处理逻辑
- [ ] 04-03: 优化历史查询接口与前端加载策略
...
| 4. 契约与前端稳态 | 0/3 | Not started | - |
| 5. Feed 性能与回归保障 | 0/3 | Not started | - |
